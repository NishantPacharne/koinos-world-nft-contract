import { System, SafeMath, Protobuf, Arrays, authority, Token, error, StringBytes, Base58 } from "@koinos/sdk-as";
...
export class Collections {
  ...
  get_timestamp(
    args: collections.get_timestamp_arguments
  ...
  mint(args: collections.mint_arguments): collections.empty_object {
    ...
    for (let index = start; index <= tokens; index++) {
      ...
      // mint token
      newToken.owner = to;
      newToken.timestamp = System.getHeadInfo().head_block_time; // Record current timestamp
  ...
  transfer(args: collections.transfer_arguments): collections.empty_object {
    ...
    token!.owner = to;
    token!.timestamp = System.getHeadInfo().head_block_time; // Record current timestamp
